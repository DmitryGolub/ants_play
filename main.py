...
#
#
# # hex координаты → декартовы (для визуализации)
...
#     #     json.dump(arena, file, indent=4, ensure_ascii=False)
#
#     # print("Finish pars")
#
...
#     ax.axis('off')
#
#     # Карта — закрасим клетки в зависимости от type
#     tile_colors = {
#         1: "#b5e7a0",  # пример: трава
#         2: "#ffdac1",  # пример: песок
#         3: "#e0bbf3",  # пример: камень
#         4: "#fdffb6"  # пример: вода
#     }
...
#         hex_patch = patches.RegularPolygon((x, y), numVertices=6, radius=1, orientation=np.radians(30),
...
#         ax.plot(x, y, marker='o', markersize=20, color='red', alpha=0.7)
#         ax.text(x, y, str(food["amount"]), va="center", ha="center", fontsize=10, color='white')
#
#     # Дом (home)
#     for home in arena.get("home", []):
#         x, y = hex_to_pixel(home["q"], home["r"])
#         ax.plot(x, y, marker='s', markersize=20, color='blue', alpha=0.5)
#
#     # Муравьи (ants)
#     ant_colors = {
#         0: "black",  # свой тип
...
#         ax.plot(x, y, marker='P', markersize=25, color=color)
#         ax.text(x, y + 0.3, f"{ant['health']}", ha="center", fontsize=9, color=color)
#
#     # Враги (если есть)
#     for enemy in arena.get("enemies", []):
#         x, y = hex_to_pixel(enemy["q"], enemy["r"])
#         ax.plot(x, y, marker='X', markersize=18, color='red')
#
#     plt.title(f"Turn: {arena.get('turnNo', 0)}  Score: {arena.get('score', 0)}")
#     plt.show()
